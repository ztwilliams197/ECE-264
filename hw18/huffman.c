...
void write_compressed(TreeNode* root, BitWriter* a_writer, const char* in_path) {
	...
	for(uint8_t curr = fgetc(in_file); !feof(in_file); curr = fgetc(in_file)) {
		write_bits(a_writer, char_to_code[curr].bits, char_to_code[curr].num_bits);
	}
