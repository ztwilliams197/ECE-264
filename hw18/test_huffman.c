...
int _test_compression() {
	mu_start();

	Frequencies gophers_freqs = { ['g'] = 3, ['o'] = 3, [' '] = 2, ['s'] = 1, ['e'] = 1, ['h'] = 1, ['p'] = 1, ['r'] = 1};
	Node* head = make_huffman_pq(gophers_freqs);
	TreeNode* root = make_huffman_tree(head);
	BitWriter a_writer = open_bit_writer("gopher_compressed.txt");
	uint8_t* uncompressed_bytes = (uint8_t*)"go go gophers";
	write_compressed(root, &a_writer, uncompressed_bytes);
	close_bit_writer(&a_writer);
	destroy_huffman_tree(&root);
